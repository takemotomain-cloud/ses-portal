  ...
  return (
    <div>
      ...
      <div className="flex gap-2 mb-4 flex-wrap items-center">
        <input
          type="text"
          placeholder="取引先名・契約名で検索"
          value={search}
          onChange={e => setSearch(e.target.value)}
          className="border border-border rounded-md px-3 py-[7px] text-sm outline-none bg-card min-w-[200px]"
        />
        <select
          value={statusFilter}
          onChange={e => setStatusFilter(e.target.value)}
          className="border border-border rounded-md px-3 py-[7px] text-sm outline-none bg-card"
        >
          {statusOptions.map(o => (
      ...
      <div className="card p-0 overflow-x-auto">
        <table className="w-full min-w-[800px]">
          <thead><tr className="border-b border-border">
            {['契約名', '取引先', '種別', 'ステータス', '契約期間', '担当', '最終更新'].map(h => (
              <th key={h} className={`text-xs text-secondary font-normal px-4 py-2.5 bg-[#FAFAFA] ${h === '契約期間' ? 'text-right' : 'text-left'}`}>{h}</th>
            ))}
          </tr></thead>
          <tbody>
            {filtered.length === 0 ? (
              ...
              return (
                <tr key={idx} className="border-b border-border/20 hover:bg-[#FAFAF8] cursor-pointer">
                  ...
                  <td className="px-4 py-2.5"><span className={`badge ${st?.cls ?? ''}`}>{st?.label ?? c.status}</span></td>
                  <td className="px-4 py-2.5 text-base text-secondary text-right">{c.period}</td>
                  <td className="px-4 py-2.5 text-base">{c.assignee}</td>
