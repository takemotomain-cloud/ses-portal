...
JSONのみを返してください。説明やマークダウンは不要です。`,
  ...
  async updateCard(id: string, data: {
    ...
    return this.db.businessCard.update({ where: { id }, data: updateData });
  }
   ...
   * 商談ログに名刺画像を追加
   */
  async addCardImageToLog(logId: string, imageBuffer: Buffer): Promise<string> {
   ...
   */
  async deleteCardImage(cardId: string): Promise<void> {
   ...
   * 物理ファイルも削除する。
   */
  async deleteLogCardImage(logId: string, imagePath: string): Promise<void> {
